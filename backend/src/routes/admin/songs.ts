...
// Upload a new song
router.post('/', authMiddleware, adminMiddleware, upload.single('file'), async (req: AuthRequest & MulterRequest, res) => {
  try {
    if (!req.file) {
    ...
    if (!uploadResponse.ok) {
      throw new Error(`Failed to upload to Bunny CDN: ${uploadResponse.statusText}`);
    }
